...
self._i = ['Anstear Phi\x67ros模拟器', [2, 2, 0, '2025 Remake'], 1621795955, 1680614770];

function drawGlowingText(text, x, y, color, glowColor, glowSize) {
    // 设置发光效果
    ctxos.shadowColor = glowColor;
    ctxos.shadowBlur = glowSize;
    
    // 绘制发光文本
    ctxos.fillStyle = color;
...
// 指定日期，这里是2022年1月1日
const specifiedDate = new Date('2025-9-4');
...
const judgeManager = {
	...
	addEvent(notes, realTime) {
		...
		list.length = 0;
		if (app.playMode === 1) {
			const dispTime = Math.min(frameTimer.disp, 0.01);
...
function loopCanvas() { //尽量不要在这里出现app
	...
	ctxos.globalAlpha = 1;
	ctxos.textAlign = 'center';
	ctxos.font = `${lineScale * 1.30}px Saira,Saira`;
	ctxos.fillText(tmps.combo, canvasos.width / 2, lineScale * 1.375); //连坤数
	ctxos.globalAlpha = qwqIn.second < 0.67 ? tween.easeOutSine(qwqIn.second * 1.5) : (1 - tween.easeOutSine(qwqOut.second * 1.5));
	ctxos.font = `${lineScale * 0.5}px Saira,Saira`;
	ctxos.fillText(tmps.combo2, canvasos.width / 2, lineScale * 1.95); //文本Combo or autoplay
	//绘制曲名和等级
	ctxos.globalAlpha = 1;
	...
	ctxos.textAlign = 'right';
	// 第一行：FPS文本（更靠上，透明度更低）
	ctxos.globalAlpha *= 0.8; // 文本透明度更低
	ctxos.font = `${lineScale * 0.32}px Saira,Saira`;
	ctxos.fillText('FPS', canvasos.width - lineScale * 0.05, lineScale * 0.4); // 位置上移

	// 第二行：帧率数字（更大，间隔更大）
	ctxos.globalAlpha = (qwqIn.second < 0.67 ? tween.easeOutSine(qwqIn.second * 1.5) : (1 - tween.easeOutSine(qwqOut.second * 1.5))) * 0.8; // 数字透明度
	ctxos.font = `${lineScale * 0.5}px Saira,Saira`;
	ctxos.fillText(frameTimer.fpsStr, canvasos.width - lineScale * 0.05, lineScale * 0.75); // 位置上移，间隔更大
	if (showStat.checked) {
		ctxos.textAlign = 'right';
...
// 检查是否达到理论值
	...
	ctxos.fillText('准确率', -1020 * tween.ease10(range(qwqEnd.second * 0.9 - 0.25)) + 2755, 670);
	ctxos.textAlign = "left";
	ctxos.font = "50px Saira,SyHybrid,Noto Sans SC";
