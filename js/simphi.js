...
class Stat {
    ...
    // 在这里定义 getTheoreticalValue 方法
    getTheoreticalValue() {
        if (app && app.playMode === 1) { // autoplay 模式
            return 1000000 + (this.numOfNotes || 0);
        }
        
        const totalJudged = (this.perfect || 0) + (this.good || 0);
        const isAllPerfect = totalJudged === (this.numOfNotes || 0);
        
        if (isAllPerfect) {
            return 1000000 + (this.maxperfect || 0);
        } else {
            return 1000000 + (this.numOfNotes || 0); // 最大可能理论值
        }
    }
...
// 修改 get RTR() 方法，使用 stat.level 而不是 selectLevel
	...
	get scoreNum() {
    // 计算总判定数（Perfect + Good）
    	const totalJudged = this.perfect + this.good;
    	const isAllPerfect = totalJudged === this.numOfNotes;

    	if (isAllPerfect) {
        // All Perfect 情况下：100万分 + 实际获得的大P数量
        	return 1000000 + this.maxperfect + this.flick + this.drag;
    	} else {
